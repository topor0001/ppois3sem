class Set:
    ...
    def from_string(cls, string_repr):
        return cls._parse_from_string(string_repr.strip())
    
    @classmethod
    def _parse_from_string(cls, string_repr, level=0):
        ...
        while pos < len(content):
            ...
            if char == '{':
                ...
                element_str = cls._chars_to_string(current_element).strip()
                if element_str:
                    if element_str.startswith('{'):
                        nested_set = cls._parse_from_string(element_str, level + 1)
                        result.add_element(nested_set)
                    else:
                        result.add_element(element_str.strip())
                        element_counter += 1
                current_element = []
            else:
                current_element.append(char)
